          ...
          <div class="col-lg-12">
            <div class="card card-primary card-outline">
              ...
              <?php 
                ...
                $salStat = $pdo -> prepare("SELECT * FROM sale_orders ORDER BY sal_id DESC");
                $salStat -> execute();
                $salResult = $salStat -> fetchAll();
                $total_pages = ceil(count($salResult) / $numOfrecs);

                $pdostat = $pdo -> prepare("SELECT * FROM sale_orders ORDER BY sal_id DESC LIMIT $offset,$numOfrecs");
                $pdostat -> execute();
                        ...
                        <tbody class="text-center">
                          <?php 
                                ...
                                if($result){
                                    ...
                                    $useStat = $pdo -> prepare("SELECT * FROM users WHERE id=".$value['sal_user_id']);
                                    $useStat -> execute();
                                    $useResult = $useStat -> fetchAll();
                                ?>
                                      <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php echo escape($useResult[0]['name']); ?></td>
                                        <td><?php echo escape($value['sal_total_price']); ?></td>
                                        <td><?php echo escape(date('Y-m-d',strtotime($value['sal_order_date']))); ?></td>
                                        <td><a href="order_detail.php?id=<?php echo $value['sal_id']; ?>"><input type="submit" value="View" class="btn btn-primary text-center"></a></td>
                                    </tr>  
